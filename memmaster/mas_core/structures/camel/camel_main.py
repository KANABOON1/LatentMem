...
from memmaster.mas_core.structures.camel.prompts import prompt
...
class CamelMemoryMAS(BaseMemoryMAS):
    ...
    def generate(self, task_domain_instructions: list[str], user_inputs: list[str], generation_config: GenerationConfig) -> list[MessageGraph]:
        ...
        # --- user proxy agent outputs ---
        userproxy_user_inputs, userproxy_system_inputs = dict(), dict()
        userproxy_user_inputs["task_description"] = user_inputs
        userproxy_system_inputs["task_domain_instructions"] = task_domain_instructions
        userproxy_messages = self.userproxy_agent.invoke(userproxy_system_inputs, userproxy_user_inputs, generation_config)
        ...
        # --- actor agent outputs ---
        actor_user_inputs, actor_system_inputs = dict(), dict()
        actor_user_inputs["task_description"] = user_inputs
        actor_user_inputs["userproxy_output"] = [msg.response for msg in userproxy_messages]
        actor_system_inputs["task_domain_instructions"] = task_domain_instructions
        actor_messages = self.actor_agent.invoke(actor_system_inputs, actor_user_inputs, generation_config)
        ...
        critic_user_inputs, critic_system_inputs = dict(), dict()
        critic_user_inputs["task_description"] = user_inputs
        critic_user_inputs["actor_output"] = [msg.response for msg in actor_messages]
        ...
        summarizer_user_inputs["actor_output"] = [msg.response for msg in actor_messages]
        summarizer_user_inputs["critic_output"] = [msg.response for msg in critic_messages]
