...
class GMemory(base_rag.BaseRAGMemory):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.neightbor_hops = neighbor_hops
        ...
        self.query_layer = QueryLayer(
            ...
            node_similarity_threshold=node_similarity_threshold
        )
    ...
    def add(self, mas_trajectory: Trajectory):
        # using llm to compress long trajectories
        # mas_message = self._extract_mas_message(mas_message=mas_message)  
        
        self.query_layer.add_task_node(mas_trajectory.task_init_description)
    ...
    def _retrieve(
        ...
        def sort_and_filter_by_similarity(docs: list[Document], threshold: float = 0.3) -> list[tuple[Document, float]]:
            ...
            for doc in docs:
                ...
                sim = utils.cosine_similarity(origin_embedding, embedding)
                if sim >= threshold:
        ...
        pos_shots_doc: list[Document] = []
        neg_shots_doc: list[Document] = []
        
        # find related tasks in query layer
        related_point_num: int = max((pos_shots_num + neg_shots_num) // 2, 1)  
        task_mains: list[str] = self.query_layer.retrieve_related_task(query_task=query, node_num=related_point_num, hop=self.neightbor_hops)
        for task_main in task_mains:
            doc = self.main_memory.similarity_search(task_main, k=1)[0]

            if doc.metadata.get('label') == True:
                pos_shots_doc.append(doc)
            elif doc.metadata.get('label') == False:
                neg_shots_doc.append(doc)
            else:
        ...
        if len(pos_shots_doc) < pos_shots_num:
            pos_shots_doc = self.main_memory.similarity_search(
                query=query, k=pos_shots_num, filter={'label': True}
            )
            for doc in pos_shots_doc:
                if doc not in pos_shots_doc:
                    pos_shots_doc.append(doc)
        
        if len(neg_shots_doc) < neg_shots_num:
            neg_shots_doc = self.main_memory.similarity_search(
                query=query, k=neg_shots_num, filter={'label': False}
            )
            for doc in neg_shots_doc:
                if doc not in neg_shots_doc:
                    neg_shots_doc.append(doc)

        # order by similarity        
        origin_embedding: list[float] = self.embedding_function.embed_query(query)
        true_tasks_doc_with_score = sort_and_filter_by_similarity(pos_shots_doc, threshold)[:pos_shots_num]
        false_tasks_doc_with_score = sort_and_filter_by_similarity(neg_shots_doc, threshold)[:neg_shots_num]
        ...
        insights_with_score = self.insight_layer.query_insights_with_score(query, top_k=insights_num)
        insights = [insight for insight, _ in insights_with_score][:insights_num]

        return true_task_messages, false_task_messages, insights

    def retrieve(self, query: str, **kwargs) -> base_rag.RAGMemoryResponse:
        # retrieve raw tasks
        pos_shots, neg_shots, insights = self._retrieve(
            query, self.pos_shots_num, self.neg_shots_num, self.insights_num
        )
    ...
    def _extract_mas_message(self, mas_message: Trajectory) -> Trajectory:
        mas_message_copy: Trajectory = copy.deepcopy(mas_message)

        user_prompt: str = prompt.GMEMORY_COMPRESS_TRAJECTORY_SYSTEM_PROMPT.format(
            task_trajectory=mas_message_copy.to_text()
        )
        conversations = [
            {"role": "system", "content": prompt.GMEMORY_COMPRESS_TRAJECTORY_SYSTEM_PROMPT},
            {"role": "user", "content": user_prompt}
        ]
        generation_config = GenerationConfig(
            **self.generation_common_settings,
            max_new_tokens=200  
        )
        response: str = utils.llm_generate(self.llm, self.tokenizer, generation_config, [conversations])[0]
        mas_message_copy.add_extra_fields("compressed_trajectory", response)
...
class QueryLayer:
    ...
    def __post_init__(self):
        ...
        self._graph_save_path: str = os.path.join(self.working_dir, f'graph.pkl')
